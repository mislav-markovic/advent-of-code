...
fn solve_part1(input: &str) -> u32 {
    ...
    let heuristic = |current| manhattan_distance(current, &map.destination_pos);
    let get_neighbours = |current| map.get_valid_neighbours(current);
...
fn a_star(
    ...
    open_set.push(Reverse(Node::with_score(
        start.clone(),
        (heuristic)(&start),
    )));

    let mut goal_score = HashMap::new();
    goal_score.insert(start.clone(), 0u32);
    ...
    }) = open_set.pop()
    {
        ...
        for neighbour in (get_neighbours)(&current) {
            ...
            let current_neighbour_score = goal_score.entry(neighbour.clone()).or_insert(Score::MAX);
